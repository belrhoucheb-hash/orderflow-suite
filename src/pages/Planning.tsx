  ...
  const { data: dbOrders = [], refetch, isLoading: ordersLoading, isError: ordersError } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("orders")
        .select("id, order_number, client_name, pickup_address, delivery_address, quantity, weight_kg, requirements, is_weight_per_unit, time_window_start, time_window_end, geocoded_pickup_lat, geocoded_pickup_lng, geocoded_delivery_lat, geocoded_delivery_lng, delivery_date, pickup_date, info_status")
        .eq("status", "PENDING")
        .is("vehicle_id", null)
