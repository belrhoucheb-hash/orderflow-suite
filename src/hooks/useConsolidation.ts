...
export function useConsolidationGroups(plannedDate: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any).from("consolidation_groups")
...
export function useConsolidationGroupOrders(groupId: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any).from("consolidation_orders")
...
export function useCreateConsolidationGroup() {
  ...
  return useMutation({
    mutationFn: async (group: Omit<ConsolidationGroup, "id" | "created_at" | "updated_at" | "orders" | "vehicle">) => {
      const { data, error } = await (supabase as any).from("consolidation_groups").insert(group).select().single();
...
export function useUpdateConsolidationGroup() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...updates }: Partial<ConsolidationGroup> & { id: string }) => {
      const { data, error } = await (supabase as any).from("consolidation_groups").update(updates).eq("id", id).select().single();
      if (error) throw error; return data as ConsolidationGroup;
    },
...
export function useAddOrderToGroup() {
  ...
  return useMutation({
    mutationFn: async ({ groupId, orderId, stopSequence }: { groupId: string; orderId: string; stopSequence: number | null }) => {
      const { data, error } = await (supabase as any).from("consolidation_orders").insert({ group_id: groupId, order_id: orderId, stop_sequence: stopSequence }).select().single();
...
export function useRemoveOrderFromGroup() {
  ...
  return useMutation({
    mutationFn: async ({ id, groupId }: { id: string; groupId: string }) => {
      const { error } = await (supabase as any).from("consolidation_orders").delete().eq("id", id);
...
export function useMoveOrderBetweenGroups() {
  ...
  return useMutation({
    mutationFn: async ({ consolidationOrderId, fromGroupId, toGroupId, newSequence }: { consolidationOrderId: string; fromGroupId: string; toGroupId: string; newSequence: number; }) => {
      const { data, error } = await (supabase as any).from("consolidation_orders").update({ group_id: toGroupId, stop_sequence: newSequence }).eq("id", consolidationOrderId).select().single();
