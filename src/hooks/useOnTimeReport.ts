...
export function calculateOnTimeMetrics(stops: StopMetric[]): OnTimeMetrics {
  ...
  for (const stop of stops) {
    switch (stop.window_status) {
      ...
      case "TE_LAAT":
        lateCount++;
        if (stop.client_location_id) {
          violationsByLocation[stop.client_location_id] = (violationsByLocation[stop.client_location_id] || 0) + 1;
        }
        break;
      case "GEMIST":
        missedCount++;
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          planned_window_end,
          actual_arrival_time,
          trip:trips!inner(planned_date, tenant_id)
        `)
