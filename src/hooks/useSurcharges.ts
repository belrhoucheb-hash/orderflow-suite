...
export function useSurcharges(activeOnly = true) {
  return useQuery({
    ...
    staleTime: 15_000,
    queryFn: async () => {
      let query = fromTable("surcharges").select("*").order("name", { ascending: true });
...
export function useCreateSurcharge() {
  ...
  return useMutation({
    mutationFn: async (input: CreateSurchargeInput) => {
      const { data, error } = await fromTable("surcharges")
...
export function useDeleteSurcharge() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { error } = await fromTable("surcharges").delete().eq("id", id);
