      ...
      <div className="rounded-xl border border-primary/15 bg-primary/5 p-4 space-y-2">
        ...
        {capacityMatches.length > 0 ? (
          <div className="space-y-2">
            ...
            <button className="w-full mt-1 py-1.5 text-[10px] font-semibold text-primary hover:bg-primary/5 rounded border border-transparent hover:border-primary/10 transition-colors">
              Alle resultaten zien
            </button>
          </div>
        ) : (
          <p className="text-[11px] text-muted-foreground bg-card rounded-lg p-3 border border-border/40 text-center">Geen geschikte voertuigen gevonden</p>
        )}
      </div>
    </div>
  );
