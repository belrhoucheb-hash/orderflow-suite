...
export function StopTimeWindow({ windowStart, windowEnd, windowStatus, waitingTimeMin }: Props) {
  ...
  const isEarly = windowStatus === "TE_VROEG";
  ...
  return (
    <div className={cn("rounded-lg border p-3 space-y-2", isLate ? "border-red-300 bg-red-50" : isOnTime ? "border-green-300 bg-green-50" : "border-gray-200")}>
      <div className="flex items-center justify-between">
        ...
        <span className={cn("text-xs font-medium px-2 py-0.5 rounded", statusInfo.color)}>
          {statusInfo.label}
        </span>
      </div>
      ...
      {remaining !== null && remaining > 0 && (
        <div className="flex items-center gap-1 text-sm text-muted-foreground">
          ...
          <span>Nog {remaining} min tot opening</span>
        </div>
      )}
      ...
      {isLate && (
        <div className="flex items-center gap-1 text-sm text-red-700">
          ...
          <span>Te laat — venster is verlopen</span>
        </div>
      )}
      ...
      {waitingTimeMin !== null && waitingTimeMin > 0 && (
        <div className="flex items-center gap-1 text-sm text-amber-700">
          <Timer className="h-3 w-3" />
          <span>Wachttijd: {waitingTimeMin} min</span>
        </div>
      )}
    </div>
  );
