...
import {
  ...
  Calendar, Barcode, FileText, Package,
} from "lucide-react";
...
export function TransportOrdersTable() {
  ...
  const filtered = useMemo(() => {
    ...
    result = [...result].sort((a, b) => {
      ...
      else if (sortField === "transport_type") cmp = (a.transport_type || "").localeCompare(b.transport_type || "");
      else if (sortField === "order_number") cmp = a.order_number - b.order_number;
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px] max-w-[320px]">
          ...
          <Input placeholder="Zoeken..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-8 text-xs" />
        </div>
        ...
        <div className="ml-auto flex items-center gap-3">
          {selectedIds.size > 0 && (
            <Badge variant="secondary" className="text-[10px]">{selectedIds.size} geselecteerd</Badge>
          )}
      ...
      {/* Table */}
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-xl border border-border/40 shadow-sm overflow-hidden">
              ...
              <tr className="border-b border-border/30 bg-muted/20">
                ...
                <th className="px-3 py-2 text-left">
                  <button onClick={() => toggleSort("created_at")} className="flex items-center gap-1 text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70 hover:text-foreground transition-colors">
                    <Calendar className="h-3 w-3" />Datum <SortIcon field="created_at" />
                  </button>
                </th>
                <th className="px-3 py-2 text-left">
                  <button onClick={() => toggleSort("transport_type")} className="flex items-center gap-1 text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70 hover:text-foreground transition-colors">
                    Transporttype <SortIcon field="transport_type" />
                  </button>
                </th>
                <th className="px-3 py-2 text-left">
                  <button onClick={() => toggleSort("client_name")} className="flex items-center gap-1 text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70 hover:text-foreground transition-colors">
                    Naam besteller <SortIcon field="client_name" />
                  </button>
                </th>
                <th className="px-3 py-2 text-left text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70 hidden lg:table-cell">Tijdstip</th>
                <th className="px-3 py-2 text-left text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70 hidden md:table-cell">Voertuig</th>
                <th className="px-3 py-2 text-left text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70 hidden lg:table-cell">Omschr.</th>
                <th className="px-3 py-2 text-left">
                  <button onClick={() => toggleSort("order_number")} className="flex items-center gap-1 text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70 hover:text-foreground transition-colors">
                    <FileText className="h-3 w-3" />Ref. factuur <SortIcon field="order_number" />
                  </button>
                </th>
                <th className="px-3 py-2 text-left text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70 hidden xl:table-cell">
                  <span className="flex items-center gap-1"><Barcode className="h-3 w-3" />Barcode</span>
                </th>
                <th className="px-3 py-2 text-left text-[10px] font-medium uppercase tracking-wider text-muted-foreground/70">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border/20">
              {filtered.map((order, i) => (
                <motion.tr
                  ...
                  className={cn("hover:bg-muted/20 transition-colors duration-100", selectedIds.has(order.id) && "bg-primary/[0.03]")}
                >
                  <td className="px-3 py-2"><Checkbox checked={selectedIds.has(order.id)} onCheckedChange={() => toggleSelect(order.id)} className="h-3.5 w-3.5" /></td>
                  <td className="px-3 py-2 text-[12px] text-foreground tabular-nums whitespace-nowrap">{formatDate(order.created_at)}</td>
                  <td className="px-3 py-2 text-[12px] text-foreground">{order.transport_type || "—"}</td>
                  <td className="px-3 py-2 text-[12px] font-medium text-foreground">{order.client_name || "Onbekend"}</td>
                  <td className="px-3 py-2 text-[12px] text-muted-foreground tabular-nums whitespace-nowrap hidden lg:table-cell">{formatTime(order.created_at)}</td>
                  <td className="px-3 py-2 text-[12px] text-muted-foreground hidden md:table-cell">
                    {order.vehicle_id ? <Badge variant="outline" className="text-[9px] px-1.5">Toegewezen</Badge> : <span className="text-muted-foreground/50">—</span>}
                  </td>
                  <td className="px-3 py-2 text-[12px] text-muted-foreground truncate max-w-[120px] hidden lg:table-cell">
                    {order.quantity && order.unit ? `${order.quantity} ${order.unit}` : order.weight_kg ? `${order.weight_kg} kg` : "—"}
                  </td>
                  <td className="px-3 py-2"><span className="text-[11px] font-mono text-foreground/80">{order.invoice_ref || "—"}</span></td>
                  <td className="px-3 py-2 hidden xl:table-cell"><span className="text-[10px] font-mono text-muted-foreground tracking-wider">{order.barcode || "—"}</span></td>
                  <td className="px-3 py-2">
                    <Badge variant="outline" className={cn("text-[9px] px-1.5 py-0.5 border", statusColors[order.status])}>
