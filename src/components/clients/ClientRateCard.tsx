...
export function ClientRateCard({ clientId, clientName }: ClientRateCardProps) {
  ...
  if (isLoading) return <LoadingState message="Tarieven laden..." />;
  ...
  const handleAddRule = (cardId: string) => {
    ...
    setEditingRules((prev) => ({
      ...
      [cardId]: [
        ...(prev[cardId] ?? []),
        { rule_type: "VAST_BEDRAG" as RuleType, transport_type: "", amount: "", min_amount: "", conditions: "{}", sort_order: (prev[cardId] ?? []).length },
      ],
    }));
  };
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Specifieke tariefkaart voor {clientName}. Zonder klant-tariefkaart wordt het standaard tarief gebruikt.
            </CardDescription>
          </div>
          {!activeCard && (
            <Button onClick={handleCreateCard}>
              <Plus className="h-4 w-4 mr-1" /> Tariefkaart aanmaken
            </Button>
          )}
        </div>
      </CardHeader>
      <CardContent>
        {cards.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-8">
            Geen klant-specifieke tariefkaart. Het standaard tarief wordt gebruikt.
          </p>
        ) : (
          cards.map((card) => {
              ...
              <div key={card.id} className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      if (confirm("Tariefkaart verwijderen?")) {
                ...
                {rows.map((rule, idx) => (
                  <div key={idx} className="grid grid-cols-12 gap-2 items-end">
                    ...
                    <div className="col-span-2">
                      {idx === 0 && <Label className="text-xs">Bedrag</Label>}
                      <Input type="number" step="0.01" value={rule.amount} onChange={(e) => handleRuleChange(card.id, idx, "amount", e.target.value)} className="h-9" />
                    </div>
                    <div className="col-span-2">
                      {idx === 0 && <Label className="text-xs">Min.</Label>}
                      <Input type="number" step="0.01" value={rule.min_amount} onChange={(e) => handleRuleChange(card.id, idx, "min_amount", e.target.value)} className="h-9" placeholder="Geen" />
                    </div>
                    <div className="col-span-2">
                      {idx === 0 && <Label className="text-xs">Transport</Label>}
                      <Input value={rule.transport_type} onChange={(e) => handleRuleChange(card.id, idx, "transport_type", e.target.value)} className="h-9" placeholder="Alle" />
                    </div>
                    <div className="col-span-2">
                      {idx === 0 && <Label className="text-xs">Condities</Label>}
                      <Input value={rule.conditions} onChange={(e) => handleRuleChange(card.id, idx, "conditions", e.target.value)} className="h-9 font-mono text-xs" />
                    </div>
                    <div className="col-span-1">
                      <Button variant="ghost" size="sm" className="h-9 w-9 p-0" onClick={() => handleRemoveRule(card.id, idx)}>
                        <Trash2 className="h-4 w-4 text-destructive" />
                      </Button>
                    </div>
                  </div>
