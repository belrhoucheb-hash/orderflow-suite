...
/**
 * Auto-size columns by calculating the max width of each column
 * (header + data rows).
 */
...
/**
 * Format numeric cells in a sheet to 2 decimal places.
 */
function formatNumberCells(
  ...
  const numericIndices = numericKeys
    .map((k) => keys.indexOf(k))
    .filter((i) => i >= 0);

  for (let R = range.s.r + 1; R <= range.e.r; R++) {
    for (const C of numericIndices) {
...
export function exportOrdersExcel(orders: ReportOrder[]): void {
  const XLSX = require("xlsx") as typeof import("xlsx");

  const data = orders.map((o) => ({
    Ordernummer: o.order_number ?? o.id,
    Aangemaakt: formatDateNL(o.created_at),
    Status: dutchStatus(o.status),
    Klant: o.client_name ?? "",
    Ophaaladres: o.pickup_address ?? "",
    Afleveradres: o.delivery_address ?? "",
    "Gewicht (kg)": o.weight_kg ?? 0,
    Aantal: o.quantity ?? 0,
  }));

  const headers = Object.keys(data[0] ?? {});
  const ws = XLSX.utils.json_to_sheet(data);
  autoSizeColumns(ws, data, headers);
  formatNumberCells(ws, data, ["Gewicht (kg)", "Aantal"]);

  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Orders");
  XLSX.writeFile(wb, `orders-export-${formatDateNL(new Date())}.xlsx`);
}

export function exportTripsExcel(trips: ReportTrip[]): void {
  const XLSX = require("xlsx") as typeof import("xlsx");

  const data = trips.map((t) => ({
    Ritnummer: t.trip_number ?? t.id,
    Datum: formatDateNL(t.date),
    Chauffeur: t.driver_name ?? "",
    Voertuig: t.vehicle_name ?? "",
    Route: t.route ?? "",
    Stops: t.stops ?? 0,
    "Afstand (km)": t.distance_km ?? 0,
    Kosten: t.cost ?? 0,
    Omzet: t.revenue ?? 0,
    Marge: t.margin ?? 0,
  }));
  ...
  const ws = XLSX.utils.json_to_sheet(data);
  autoSizeColumns(ws, data, headers);
  formatNumberCells(ws, data, ["Afstand (km)", "Kosten", "Omzet", "Marge"]);

  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Ritten");
  XLSX.writeFile(wb, `ritten-export-${formatDateNL(new Date())}.xlsx`);
}

export function exportFinancialExcel(invoices: ReportInvoice[]): void {
  const XLSX = require("xlsx") as typeof import("xlsx");

  const data = invoices.map((inv) => ({
    Factuurnummer: inv.invoice_number,
    Klant: inv.client_name,
    Factuurdatum: formatDateNL(inv.invoice_date),
    Vervaldatum: inv.due_date ? formatDateNL(inv.due_date) : "",
  ...
  const ws = XLSX.utils.json_to_sheet(data);
  autoSizeColumns(ws, data, headers);
  formatNumberCells(ws, data, ["Subtotaal", "BTW", "Totaal"]);

  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Facturen");
  XLSX.writeFile(wb, `facturen-export-${formatDateNL(new Date())}.xlsx`);
}
