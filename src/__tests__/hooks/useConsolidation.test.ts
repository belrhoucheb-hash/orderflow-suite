...
const { mockSelect, mockInsert, mockUpdate, mockDelete, mockEq, mockOrder, mockFrom, mockSupabase } = vi.hoisted(() => {
  ...
  const mockSupabase = { from: mockFrom, auth: { getSession: vi.fn().mockResolvedValue({ data: { session: null }, error: null }), onAuthStateChange: vi.fn().mockReturnValue({ data: { subscription: { unsubscribe: vi.fn() } } }) } };
...
describe("useConsolidationGroups", () => {
  ...
  it("fetches groups for a given date", async () => {
    mockOrder.mockResolvedValueOnce({ data: [{ id: "g1", name: "Regio Amsterdam", planned_date: "2026-04-04", status: "VOORSTEL" }], error: null });
    const { result } = renderHook(() => useConsolidationGroups("2026-04-04"), { wrapper: createWrapper() });
  ...
  it("is disabled when date is null", () => {
    const { result } = renderHook(() => useConsolidationGroups(null), { wrapper: createWrapper() });
