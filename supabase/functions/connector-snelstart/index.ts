// Connector-snelstart: nieuwe gateway voor Snelstart, gebouwd op de
// connector-runtime. Vervangt op termijn snelstart-sync (die blijft als
// legacy-alias voor de huidige frontend-invoke-flow).
//
// Aanroep:
//   POST { action: 'push'|'test', tenant_id, event_type?, payload? }
//
// Push doet hetzelfde als snelstart-sync maar via runtime, met sync-log.
...
Deno.serve(async (req) => {
  ...
  if (body.action === "test") {
    const result = await runConnectorAction(
      ...
      () => SnelstartConnector.testConnection(config, supabase),
    );
  ...
  if (body.action === "push") {
    ...
    const result = await runConnectorAction(
      ...
      () => SnelstartConnector.push(eventType, payload, config, supabase),
    );
    return new Response(JSON.stringify(result), { status: 200, headers });
  }
