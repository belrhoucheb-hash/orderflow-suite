 ...
 *
 * Pure functions, geen Supabase-dependency. Gedeeld tussen frontend en Edge Function.
 * Berekent orderprijs uit rate card rules en toeslagen.
 *
 * Afrondingsbeleid (R31): intern werkt de motor in EUR met 2 decimalen via round2.
 * Cents-conversie (×100, Math.round) gebeurt pas bij snapshot-serialisatie in pricingSnapshot.ts.
 * Timezone voor tijd-toeslagen: Europe/Amsterdam (R9), zie surchargeApplies in timeWindow.ts.
 */
...
import { RULE_TYPE_UNITS } from "./rateModels.ts";
import { surchargeMatchesTime } from "./timeWindow.ts";
...
function matchesVehicleType(rule: RateRule, order: PricingOrderInput): boolean {
  if (!rule.vehicle_type_id) return true;
...
function surchargeApplies(surcharge: Surcharge, order: PricingOrderInput): boolean {
  ...
  if (!surchargeMatchesTime(surcharge, order)) return false;
...
export function calculateWithConfidence(
  ...
  const surchargePenalty = Math.min(appliedSurchargeCount * 3, 15);
