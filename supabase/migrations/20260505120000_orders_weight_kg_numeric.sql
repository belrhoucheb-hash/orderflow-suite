-- orders.weight_kg: integer -> numeric(10,2)
--
-- Aanleiding: weight_kg was integer, waardoor decimale gewichten (luchtvracht
-- bijvoorbeeld 1247,5 kg) bij INSERT geruisloos werden afgekapt. Zowel de
-- kolom als de commit_order_draft_v1 RPC casten naar integer; beide moeten
-- mee. Index idx_orders_tenant_weight_created_at_id_desc wordt door Postgres
-- automatisch herbouwd op de nieuwe kolomtype.

ALTER TABLE public.orders
  ALTER COLUMN weight_kg TYPE numeric(10, 2)
  USING weight_kg::numeric(10, 2);
...
BEGIN
  ...
  LOOP
    ...
    VALUES (
      ...
      v_source,
      v_status,
      NULLIF(p_booking->>'weight_kg', '')::numeric,
...
COMMENT ON FUNCTION public.commit_order_draft_v1(UUID, UUID, TIMESTAMPTZ, JSONB, JSONB, JSONB, JSONB, TEXT) IS
  'Transactionele commit van order_draft naar shipment + order-legs. Lockt de draft, checkt updated_at/readiness, maakt shipment/orders en zet de immutable commit snapshot. weight_kg wordt als numeric opgeslagen om decimale gewichten (zoals luchtvracht) te behouden.';
