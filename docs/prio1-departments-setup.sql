...
--  COPY-PASTE READY voor Supabase SQL Editor
--  Project: kdszxexyefsbssfhnwxc (RCS)
--
--  1. Open https://supabase.com/dashboard/project/kdszxexyefsbssfhnwxc/sql
--  2. Klik "New query"
--  3. Plak ALLES hieronder
--  4. Klik "Run"
--
--  Veilig om meerdere keren te draaien (idempotent via IF NOT EXISTS).
-- ════════════════════════════════════════════════════════════════════════
...
DROP POLICY IF EXISTS "Service role full access on shipments" ON public.shipments;
...
DROP POLICY IF EXISTS "Service role full access on traject_rules" ON public.traject_rules;
...
-- ─── 6. Seed: Operations + Export per tenant ───────────────────────────
...
-- ─── 7. Seed: 3 baseline traject-regels per tenant ─────────────────────

-- Regel 1: naar RCS Export hub → split in 2 legs (Ops + Export)
...
-- Regel 2: vanuit RCS hub → single Export leg
INSERT INTO public.traject_rules (tenant_id, name, priority, match_conditions, legs_template)
SELECT
  t.id,
...
-- Regel 3 (fallback): alles anders → single Operations leg
INSERT INTO public.traject_rules (tenant_id, name, priority, match_conditions, legs_template)
SELECT
  t.id,
...
-- ─── 8. Backfill: bestaande orders krijgen default Operations ──────────
...
-- ─── 9. Comments (zichtbaar in Supabase schema-viewer) ─────────────────
...
--   SELECT name, priority FROM public.traject_rules ORDER BY priority;
--   SELECT COUNT(*) FROM public.orders WHERE department_id IS NULL;
-- ════════════════════════════════════════════════════════════════════════

-- ─── OPTIONEEL: Hub-adres instellen ────────────────────────────────────
-- Zonder dit gebruikt de router de fallback "Royalty Cargo Solutions, Schiphol".
-- Vervang <TENANT_ID> door jouw tenant UUID:
--
-- UPDATE public.tenants
-- SET settings = jsonb_set(
--   COALESCE(settings, '{}'::jsonb),
--   '{rcs_hub_address}',
--   '"Royalty Cargo Solutions, Folkstoneweg 40, Schiphol"'::jsonb
-- )
-- WHERE id = '<TENANT_ID>';
